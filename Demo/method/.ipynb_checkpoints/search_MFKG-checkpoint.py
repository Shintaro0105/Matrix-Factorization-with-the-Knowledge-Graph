...
#item_metapaths = ['bub_0.8.embedding', 'bcib_0.8.embedding', 'bcab_0.8.embedding']
    ...
    alldata_u=[]
    alldata_v=[]
    for reg_u in [0.0, 0.1, 0.5, 1.0]:
        for reg_v in [0.0, 0.1, 0.5, 1.0]:
            ...
            for u in range(unum):
                ...
                while n<20:
                    if R_t[u,int(pred_rating_user_item[i])]==0:
                        ...
                        i+=1
            df = pd.DataFrame(data=data, columns=["userID", "itemID", "prediction"])
            precision10 = precision_at_k(test, df, col_prediction='prediction', k=10)
            recall10 = recall_at_k(test, df, col_prediction='prediction', k=10)
            precision20 = precision_at_k(test, df, col_prediction='prediction', k=20)
            recall20 = recall_at_k(test, df, col_prediction='prediction', k=20)
            
            pred_rating_uv=np.zeros([unum,inum])
            pred_rating_uv=MYP.sigmod(MYP.mu*MU + A.dot(_A.T) + _B.dot(B.T) + MYP.U.dot(MYP.V.T))
            data = []
            for u in range(unum):
                pred_rating_user_item = np.argsort(pred_rating_uv[u,:])[::-1][:]
                n=0
                i=0
                while n<20:
                    if R_t[u,int(pred_rating_user_item[i])]==0:
                        data.append([int(u)+1,int(pred_rating_user_item[i])+1,pred_rating_uv[u,pred_rating_user_item[i]]])
                        n+=1
                        i+=1
                    else:
                        i+=1
            all_predictions_uv = pd.DataFrame(data=data, columns=["userID", "itemID", "prediction"])
            eval_precision10_uv = precision_at_k(test, all_predictions_uv, col_prediction='prediction', k=10)
            eval_recall10_uv = recall_at_k(test, all_predictions_uv, col_prediction='prediction', k=10)
            eval_precision20_uv = precision_at_k(test, all_predictions_uv, col_prediction='prediction', k=20)
            eval_recall20_uv = recall_at_k(test, all_predictions_uv, col_prediction='prediction', k=20)
            
            pred_rating_u=np.zeros([unum,inum])
            pred_rating_u=MYP.sigmod(MYP.reg_u * eu.dot(MYP.H.T))
            data = []
            for u in range(unum):
                pred_rating_user_item = np.argsort(pred_rating_u[u,:])[::-1][:]
                n=0
                i=0
                while n<20:
                    if R_t[u,int(pred_rating_user_item[i])]==0:
                        data.append([int(u)+1,int(pred_rating_user_item[i])+1,pred_rating_u[u,pred_rating_user_item[i]]])
                        n+=1
                        i+=1
                    else:
                        i+=1
            all_predictions_u = pd.DataFrame(data=data, columns=["userID", "itemID", "prediction"])
            eval_precision10_u = precision_at_k(test, all_predictions_u, col_prediction='prediction', k=10)
            eval_recall10_u = recall_at_k(test, all_predictions_u, col_prediction='prediction', k=10)
            eval_precision20_u = precision_at_k(test, all_predictions_u, col_prediction='prediction', k=20)
            eval_recall20_u = recall_at_k(test, all_predictions_u, col_prediction='prediction', k=20)
            
            pred_rating_v=np.zeros([unum,inum])
            pred_rating_v=MYP.sigmod(MYP.reg_v * MYP.E.dot(ev.T))
            data = []
            for u in range(unum):
                pred_rating_user_item = np.argsort(pred_rating_v[u,:])[::-1][:]
                n=0
                i=0
                while n<20:
                    if R_t[u,int(pred_rating_user_item[i])]==0:
                        data.append([int(u)+1,int(pred_rating_user_item[i])+1,pred_rating_v[u,pred_rating_user_item[i]]])
                        n+=1
                        i+=1
                    else:
                        i+=1
            all_predictions_v = pd.DataFrame(data=data, columns=["userID", "itemID", "prediction"])
            eval_precision10_v = precision_at_k(test, all_predictions_v, col_prediction='prediction', k=10)
            eval_recall10_v = recall_at_k(test, all_predictions_v, col_prediction='prediction', k=10)
            eval_precision20_v = precision_at_k(test, all_predictions_v, col_prediction='prediction', k=20)
            eval_recall20_v = recall_at_k(test, all_predictions_v, col_prediction='prediction', k=20)
            
            alldata.append([num_negatives,reg_u,reg_v,precision10,recall10,precision20,recall20,MYP.endstep,MYP.endloss])
            alldata_uv.append([num_negatives,reg_u,reg_v,eval_precision10_uv,eval_recall10_uv,eval_precision20_uv,eval_recall20_uv])
            alldata_u.append([num_negatives,reg_u,reg_v,eval_precision10_u,eval_recall10_u,eval_precision20_u,eval_recall20_u])
            alldata_v.append([num_negatives,reg_u,reg_v,eval_precision10_v,eval_recall10_v,eval_precision20_v,eval_recall20_v])
    result=pd.DataFrame(data=alldata, columns=["num_negatives", "reg_u", "reg_v", "pre10", "rec10", "pre20", "rec20","step","loss"])
    result_uv=pd.DataFrame(data=alldata_uv, columns=["num_negatives", "reg_u", "reg_v", "pre10", "rec10", "pre20", "rec20"])
    result_u=pd.DataFrame(data=alldata_u, columns=["num_negatives", "reg_u", "reg_v", "pre10", "rec10", "pre20", "rec20"])
    result_v=pd.DataFrame(data=alldata_v, columns=["num_negatives", "reg_u", "reg_v", "pre10", "rec10", "pre20", "rec20"])
    result.to_csv('dim128-30-30neg1.csv')
    result_uv.to_csv('dim128-30-30neg1_uv.csv')
    result_u.to_csv('dim128-30-30neg1_u.csv')
    result_v.to_csv('dim128-30-30neg1_v.csv')